...
Tjin_df = pd.read_csv (r'datos evaporador\tjin_evap3.csv', header = None) # CSV data is °C
...
for k in range(t.size):
	...
	Fvin_t[k] = evaporator_roberts.mass_vapor_in(Tjout, Pvin, vapor_prpty.temperature(Pvin), Uk, A)/vapor_prpty.density(Pvin)
...
axarr[0,0].plot(tt, sp_brx*np.ones(t.shape), 'r--', label='Setpoint', linewidth=1.5)
